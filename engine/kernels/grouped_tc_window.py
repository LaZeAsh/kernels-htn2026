"""Four-token B1 causal GQA over the full KV prefix with precise PV."""
...
def grouped_tc_window(query: torch.Tensor, keys: torch.Tensor,
    ...
    """Full-prefix GQA for four query positions at batch one."""
    if query.dtype != torch.bfloat16 or query.shape != (1, 32, 4, 128):
        raise ValueError("Q must be BF16 [1,32,4,128]")
    batch = query.shape[0]
