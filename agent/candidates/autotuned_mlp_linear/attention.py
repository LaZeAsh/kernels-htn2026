...
def _decode_linear(x, weight):
    """One shared compiled BF16 projection for every gate/up layer weight."""
...
def _mlp_forward(self, x):
    if self.decode_mode:
        if x.shape[0] <= 16:
            gate = _decode_linear(x, self.gate_proj.weight)
            up = _decode_linear(x, self.up_proj.weight)
        else:
