...
def _window_forward(model, token_ids, cache, positions):
    ...
    base = model.model
    hidden = base.embed_tokens(token_ids)
    position_embeddings = base.rotary_emb(hidden, positions)
    batch = token_ids.shape[0]
    residual = hidden.reshape(batch * 4, 1, 2560)
    layers = base.layers
    normalized = layers[0].input_layernorm(residual)
    for index, layer in enumerate(layers):
        attention = layer.self_attn(
            normalized.reshape(batch, 4, 2560),
            position_embeddings=position_embeddings,
            attention_mask=None, past_key_value=cache,
            cache_position=positions,
        )[0].reshape(batch * 4, 1, 2560)
        after_attention, mlp_input = add_norm(
            residual, attention,
            layer.post_attention_layernorm.weight,
            layer.post_attention_layernorm.variance_epsilon,
    ...
    return model.lm_head(normalized.reshape(batch, 4, 2560)).argmax(-1)
...
class Engine:
    ...
    def _prepare(self, batch, prompt_length, output_length):
        ...
        self._cache = FixedCache(
            ...
            prompt_length + output_length + (4 if batch <= 4 else 0), head_dim,
        )
        ...
        if batch <= 4:
            self._window_input = torch.empty((batch, 4), device="cuda:0", dtype=torch.int64)
            self._window_base = torch.empty((batch, 1), device="cuda:0", dtype=torch.int64)
    ...
    def _window_step(self):
        positions = self._window_base + self._window_offsets
        return _window_forward(self.model, self._window_input, self._cache, positions)

    def _capture_window(self, currents, positions):
        ...
        try:
            ...
            with torch.cuda.stream(stream):
                self._window_input.copy_(torch.tensor(
                    [[token] * 4 for token in currents],
                    device="cuda:0", dtype=torch.int64,
                ))
                self._window_base.copy_(torch.tensor(
                    positions, device="cuda:0", dtype=torch.int64,
                ).view(-1, 1))
                self._window_step()
    ...
    def generate(self, input_ids: list[list[int]], max_new_tokens: int):
        ...
        with torch.inference_mode():
            ...
            if batch <= 4:
                ...
                current_tokens = current[:, 0].tolist()
                if self._window_graph is None:
                    self._capture_window(current_tokens, [prompt_length] * batch)
                guesses = [[token] * 3 for token in current_tokens]
                positions = [prompt_length] * batch
                generated = [1] * batch
                queues = [deque() for _ in range(batch)]
                emitted_steps = 1
                while emitted_steps < max_new_tokens:
                    if all(queues):
                        yield [queue.popleft() for queue in queues]
                        emitted_steps += 1
                        continue
                    # All unfinished sequences verify in parallel, including
                    # those with queued outputs; finished rows keep their inputs.
                    self._window_input.copy_(torch.tensor(
                        [[current_tokens[b], *guesses[b]] for b in range(batch)],
                        dtype=torch.int64,
                        device="cuda:0",
                    ))
                    self._window_base.copy_(torch.tensor(
                    ...
                    for b, outputs in enumerate(self._window_graph_output.tolist()):
                        if generated[b] == max_new_tokens:
                            continue
                        accepted = 1
                        while (accepted < 4
                               and guesses[b][accepted - 1] == outputs[accepted - 1]):
                            accepted += 1
                        count = min(accepted, max_new_tokens - generated[b])
                        queues[b].extend(outputs[:count])
                        generated[b] += count
                        if generated[b] < max_new_tokens:
                            current_tokens[b] = outputs[accepted - 1]
                            guesses[b] = (outputs[accepted:]
